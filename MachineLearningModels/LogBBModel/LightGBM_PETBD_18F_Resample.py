...
log = DataLogger(log_file=f"../../data/logBB_data/log/{datetime.now().strftime('%Y%m%d')}.log").getlog("petbd_lightgbm_18F_training")
...
if __name__ == '__main__':
    log.info("===============StartingPETBD[Chinese text removed]LightGBM 18Fresampling training===============")

    petbd_data_file = "../../data/PTBD_v20240912.csv"
    petbd_features_file = "../../data/logBB_data/petbd_18F_balanced_features.csv"
    feature_index_file = "../../data/logBB_data/petbd_18F_feature_index.txt"
    ...
    log.info("[Chinese text removed]Mordred[Chinese text removed]（[Chinese text removed]）")
    X = X_mordred
    ...
    log.info("StartingLightGBM[Chinese text removed]parameter[Chinese text removed]")
    ...
    for fold, (train_idx, val_idx) in tqdm(enumerate(cv.split(X_train_val, y_train_val)), 
        ...
        y_pred_fold = model.predict(X_fold_val, num_iteration=model.best_iteration)
    ...
    model_path = "./logbbModel/lightgbm_petbd_18F_model.joblib"
    ...
    results.to_csv('./result/lightgbm_petbd_18F_results.csv', index=False)
    ...
    plt.title('LightGBM PETBD 18F-Resampled Model: Predicted vs Experimental logBB')
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig('./result/lightgbm_petbd_18F_scatter.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open('./result/lightgbm_petbd_18F_report.json', 'w', encoding='utf-8') as f:
        json.dump(experiment_report, f, ensure_ascii=False, indent=2)

    log.info("========ExperimentComplete========")
    log.info(f"[Chinese text removed]: ./result/lightgbm_petbd_18F_results.csv")
    log.info(f"Scatter plot saved to: ./result/lightgbm_petbd_18F_scatter.png") 
    log.info(f"Experiment[Chinese text removed]: ./result/lightgbm_petbd_18F_report.json")
