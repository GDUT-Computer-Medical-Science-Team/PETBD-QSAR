...
log = DataLogger(log_file=f"../../data/b3db_data/log/{datetime.now().strftime('%Y%m%d')}.log").getlog("b3db_xgb_18F_full")
...
def main():
    log.info("="*80)
    log.info("Starting B3DB Full Dataset Validation with XGBoost 18F Combined Model")
    log.info("="*80)
    ...
    results_df.to_csv('./result/B3DB_XGB_18F_Combined_Full_Predictions.csv', index=False)
    log.info("All predictions saved to './result/B3DB_XGB_18F_Combined_Full_Predictions.csv'")
    ...
    if n_with_logbb > 0:
        ...
        print("\n" + "="*80)
        print("B3DB External Validation Results (XGBoost 18F Combined)")
        print("="*80)
        ...
        plt.scatter(y_true, y_pred_eval, alpha=0.6, s=50, edgecolors='black', linewidths=0.5)
        ...
        plt.tight_layout()
        plot_file = './result/B3DB_XGB_18F_Combined_Full_Scatter.png'
        ...
        report_file = './result/B3DB_XGB_18F_Combined_Full_Report.json'
