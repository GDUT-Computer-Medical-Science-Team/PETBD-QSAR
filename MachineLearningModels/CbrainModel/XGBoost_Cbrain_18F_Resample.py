...
if __name__ == '__main__':
    ...
    train_predictions_df.to_csv('./result/xgb_cbrain_18F_train_predictions.csv', index=False)
    ...
    val_predictions_df.to_csv('./result/xgb_cbrain_18F_val_predictions.csv', index=False)
    ...
    with open('./result/xgb_cbrain_18F_report.json', 'w', encoding='utf-8') as f:
        json.dump(experiment_report, f, ensure_ascii=False, indent=2)
    
    log.info("========实验完成========")
    log.info(f"预测结果已保存至: ./result/xgb_cbrain_18F_results.csv")
    log.info(f"散点图已保存至: ./result/xgb_cbrain_18F_scatter.png") 
    log.info(f"实验报告已保存至: ./result/xgb_cbrain_18F_report.json")
