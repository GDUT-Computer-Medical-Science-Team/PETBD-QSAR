...
# Initialize logger
log = DataLogger(log_file=f"../../data/logBB_data/log/{datetime.now().strftime('%Y%m%d')}.log").getlog("cbrain_rf_xgb_18F_combined")
...
def main():
    ...
    # Dataset split (use fixed random_state=42 for consistent train/test split across runs)
    ...
    log.info(f"Starting Random Forest hyperparameter optimization ({n_optuna_trial} trials)")
    study_rf = optuna.create_study(direction='maximize')
    study_rf.optimize(lambda trial: objective_rf(trial, X_train_val, y_train_val, seed), 
                      n_jobs=4, n_trials=n_optuna_trial)
    
    best_params_rf = study_rf.best_params
    best_params_rf['random_state'] = seed
    log.info(f"Best RF parameters found: {best_params_rf}")
    log.info(f"Best RF R2 score: {study_rf.best_value:.4f}")
    ...
    log.info(f"Starting XGBoost hyperparameter optimization ({n_optuna_trial} trials)")
    study_xgb = optuna.create_study(direction='maximize')
    study_xgb.optimize(lambda trial: objective_xgb(trial, X_train_val, y_train_val, seed), 
                       n_jobs=4, n_trials=n_optuna_trial)
    
    best_params_xgb = study_xgb.best_params
    best_params_xgb['random_state'] = seed
    log.info(f"Best XGBoost parameters found: {best_params_xgb}")
    log.info(f"Best XGBoost R2 score: {study_xgb.best_value:.4f}")
