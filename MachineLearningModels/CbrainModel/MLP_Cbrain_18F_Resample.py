...
# 初始化日志
log = DataLogger(log_file=f"../../data/logBB_data/log/{datetime.now().strftime('%Y%m%d')}.log").getlog("cbrain_mlp_18F_training")
...
if __name__ == '__main__':
    log.info("===============启动Cbrain数据集MLP 18F重采样训练===============")
    
    # 文件路径配置 - 使用PETBD数据集
    cbrain_data_file = "../../data/PTBD_v20240912.csv"
    cbrain_features_file = "../../data/logBB_data/cbrain_mlp_petbd_18F_balanced_features.csv"
    feature_index_file = "../../data/logBB_data/cbrain_mlp_petbd_18F_feature_index.txt"
    ...
    if not os.path.exists(cbrain_data_file):
        raise FileNotFoundError(f"缺失Cbrain数据集: {cbrain_data_file}")
    
    log.info("读取Cbrain数据集")
    df = pd.read_csv(cbrain_data_file, encoding='utf-8')
    log.info(f"原始数据形状: {df.shape}")
    
    # 删除brain mean60min缺失的行
    ...
    def objective(trial):
        ...
        if params['solver'] == 'adam':
            params['learning_rate_init'] = trial.suggest_float('learning_rate_init', 1e-5, 1e-2, log=True)
    ...
    study.optimize(objective, n_jobs=2, n_trials=n_optuna_trial)
    ...
    hidden_layers = []
    for i in range(n_layers):
        layer_size = best_params.pop(f'layer_{i}_size')
        hidden_layers.append(layer_size)
    ...
    # 保存模型和scaler
    model_path = "./cbrainmodel/mlp_cbrain_18F_model.joblib"
    scaler_path = "./cbrainmodel/mlp_cbrain_18F_scaler.joblib"
    joblib.dump(final_model, model_path)
    joblib.dump(scaler_final, scaler_path)
    log.info(f"模型已保存至: {model_path}")
    log.info(f"缩放器已保存至: {scaler_path}")
    ...
    results.to_csv('./result/mlp_cbrain_18F_results.csv', index=False)
    ...
    train_predictions_df.to_csv('./result/mlp_cbrain_18F_train_predictions.csv', index=False)
    ...
    val_predictions_df.to_csv('./result/mlp_cbrain_18F_val_predictions.csv', index=False)
    ...
    test_predictions_df.to_csv('./result/mlp_cbrain_18F_test_predictions.csv', index=False)
    
    # 保存合并的预测结果文件
    all_predictions_df = pd.concat([train_predictions_df, val_predictions_df, test_predictions_df], 
                                   ignore_index=True)
    all_predictions_df.to_csv('./result/mlp_cbrain_18F_all_predictions.csv', index=False)
    ...
    plt.title('MLP Cbrain 18F-Resampled Model: Predicted vs Experimental Brain Concentration')
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig('./result/mlp_cbrain_18F_scatter.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open('./result/mlp_cbrain_18F_report.json', 'w', encoding='utf-8') as f:
        json.dump(experiment_report, f, ensure_ascii=False, indent=2)
    
    log.info("========实验完成========")
    log.info(f"预测结果已保存至: ./result/mlp_cbrain_18F_results.csv")
    log.info(f"散点图已保存至: ./result/mlp_cbrain_18F_scatter.png") 
    log.info(f"实验报告已保存至: ./result/mlp_cbrain_18F_report.json")
