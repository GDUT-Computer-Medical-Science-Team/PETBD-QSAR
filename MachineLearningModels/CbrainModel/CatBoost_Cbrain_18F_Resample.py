...
# 初始化日志
log = DataLogger(log_file=f"../../data/logBB_data/log/{datetime.now().strftime('%Y%m%d')}.log").getlog("cbrain_catboost_18F_training")
...
if __name__ == '__main__':
    log.info("===============启动Cbrain数据集CatBoost 18F重采样训练===============")
    
    # 文件路径配置 - 使用PETBD数据集
    cbrain_data_file = "../../data/PTBD_v20240912.csv"
    cbrain_features_file = "../../data/logBB_data/cbrain_catboost_petbd_18F_balanced_features.csv"
    feature_index_file = "../../data/logBB_data/cbrain_catboost_petbd_18F_feature_index.txt"
    ...
    # 保存模型
    model_path = "./cbrainmodel/catboost_cbrain_18F_model.joblib"
    ...
    results.to_csv('./result/catboost_cbrain_18F_results.csv', index=False)
    ...
    plt.title('CatBoost Cbrain 18F-Resampled Model: Predicted vs Experimental Brain Concentration')
    plt.legend()
    ...
    plt.tight_layout()
    plt.savefig('./result/catboost_cbrain_18F_scatter.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    with open('./result/catboost_cbrain_18F_report.json', 'w', encoding='utf-8') as f:
        json.dump(experiment_report, f, ensure_ascii=False, indent=2)
    
    log.info("========实验完成========")
    log.info(f"预测结果已保存至: ./result/catboost_cbrain_18F_results.csv")
    log.info(f"散点图已保存至: ./result/catboost_cbrain_18F_scatter.png") 
    log.info(f"实验报告已保存至: ./result/catboost_cbrain_18F_report.json")
